...
setInterval(function () {
  ...
  if (message.startsWith("!lastfm top ")) {
    ...
    $.getJSON(
      ...
      function (data) {
        msgBox.append(`${botTitle}${cmd}'s top 5 tracks are:</span></div>`);
        msgBox.append(
    ...
    msgBox.append(`${botTitle}Usage: !lastfm [top] [username]</span></div>`);
  }
  ...
  if (message.startsWith("!neko ")) {
    ...
    if (allowedEndpoints.includes(cmd)) {
      $.getJSON(`https://nekos.life/api/v2/img/${cmd}`, function (data) {
        msgBox.append(`${botTitle}Here's the ${cmd} you wanted!</span></div>`);
      ...
      msgBox.append(`${botTitle}The query ${cmd} is invalid.</span></div>`);
    }
  } else if (message == "!neko") {
    $.getJSON(`https://nekos.life/api/v2/img/neko`, function (data) {
      msgBox.append(`${botTitle}Here's the neko you wanted!</span></div>`);
      msgBox.append(`<img src="${data.url}" width="128px"></img>`);
